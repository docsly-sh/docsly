...
const MENU_ITEMS: [string, (url: string) => React.ReactNode][] = [
  ...
  [
    ...
    (url) => (
      ...
      >
        <V0Icon />
        <span className="-translate-x-px">Open in v0</span>
      </a>
    ),
  ],
  [
...
export const DocsCopyPage = ({
  ...
  const trigger = (
    <Button
      ...
      className={cn("peer -ml-0.5", docsToolbarButtonClass)}
    >
    ...
    <Popover>
      <div className="group/buttons relative flex rounded-lg bg-secondary *:data-[slot=button]:focus-visible:relative *:data-[slot=button]:focus-visible:z-10">
        ...
        <CopyButton
          ...
          className={docsToolbarButtonClass}
        >
          Copy Page
        </CopyButton>
        <DropdownMenu>
        ...
        <Separator
          ...
          className="absolute top-1 right-8 z-0 h-6! bg-foreground/5! peer-focus-visible:opacity-0"
        />
